...
<svg
  ...
  <g
    ...
    def drawOutRect(self, x1, y1, wid, hei, fill=(255, 255, 255), outfill=(0, 0, 0), lt=1, alpha=1.0, alpha2=1.0):
        ...
        self.out += '        x="%d" y="%d" width="%d" height="%d" rx="5" ry="5" />\n' % (x1, y1, wid, hei)
...
domain_dict = {}
with open(sys.argv[3]) as domains:
    for line in domains:
        if line.startswith('Q#'):
            query, hit, pssmid, fr, to, evalue, butscore, acc, short_name, incomplete, superfam = line.split('\t')
            query = query.split('>')[1]
            if query in domain_dict:
                domain_dict[query].append((int(fr), int(to), short_name))
            else:
                domain_dict[query] = [(int(fr), int(to), short_name)]
...
for num, i in enumerate(order_list):
    ...
    for j in len_dict:
        if i in j:
            svg.drawLine(curr_x, num * gap_height, curr_x + len_dict[j], num * gap_height, th=10)
            query_name = j
            for k in domain_dict[j]:
                start, stop, name = k
                if name in color_dict:
                    color = color_dict[name]
                else:
                    color = color_list[clm]
                    color_dict[name] = color
                    clm += 1
                svg.drawOutRect(curr_x + start, num * gap_height - domain_height/2, stop - start, domain_height, fill=color)
            curr_x += len_dict[j] + 50
